...
**Follow-up (2026-07-22): the baseline this issue calls "genuinely fast" (1,775 ms) was itself measuring the wrong thing.** A user manually checking the app found Consult taking far longer than that figure. Investigation confirmed every Consult test (including this one's own `041`) stopped watching the instant the "report is being generated" progress bar appeared, never waiting for it to disappear and the report to actually render - so the real click-to-rendered-report duration (~20-25s, see the new T6b tier in `specs/performance-sla.md`) had never been measured or reported anywhere. It had either vanished into an untimed `waitForLoadState('networkidle')` call in the PDF-generation tests, or silently inflated their PDF Generation timer instead, depending on run-to-run timing luck.

To try to reproduce the user's report of a multi-minute Consult, 12 back-to-back Consult clicks were run in a single isolated session (no other load) - all 12 landed in a tight 20.0-25.5s band with no degradation trend. So the multi-minute case was **not** reproduced by repetition alone; this issue's original tail-latency finding (which required genuine sustained *concurrent* load across many prior tests, not serial repetition from one session) remains the best-supported explanation, but is still unconfirmed as the specific cause of what the user saw. `041`, `042`, and `055` now all measure and assert the full render duration explicitly rather than stopping at the progress bar.
