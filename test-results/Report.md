...
- **Cycle 6 (2026-07-24): API-Exposure Security Review (new scope, FULL Step 8 mini-cycle).** Tested every discovered admin-only endpoint and several cross-tenant resource IDs against the EMPLOYE and CLIENT roles for broken access control and IDOR, tampered a live JWT's signature to confirm the backend actually verifies it, and checked CORS origin handling. **One confirmed broken-access-control bug found:** `GET /api/group-dataclass` is reachable by any authenticated role regardless of permission (both EMPLOYE and CLIENT can call it directly despite having zero UI path to it), while three sibling admin-only endpoints correctly return 403 for the same accounts - severity Low-Medium since the exposed data is only internal classification labels, no PII/financial data. No IDOR found on any account/report resource. Two informational hardening recommendations logged (login response body redundantly exposes both JWTs in plaintext alongside the proper HttpOnly cookies; auth cookies lack the `Secure` flag and no HSTS header is sent). New tests: `tests/fapa-test/security/069`-`071` (9 tests, all passing). See §3j for full detail.
...
| Stage | Passed | Failed | Total |
|---|---|---|---|
| Security suite (`069`-`071`, chromium, single worker) | 9 | 0 | 9 |
...
- **New area, cycle 6: API-Exposure Security Review.** First-ever authorization/exposure-focused testing pass in this project (everything before this was performance-timing-focused). Covers RBAC enforcement across 4 admin-only endpoints for both non-admin roles, cross-tenant IDOR checks on account/report resources, JWT signature-integrity verification, and CORS origin handling - 9 new tests, all passing. Gap: only 4 of the app's many admin-only endpoints were RBAC-tested (the ones already catalogued from performance work); a broader sweep of the full endpoint catalog in `specs/performance-sla.md` against both non-admin roles would be a natural next pass, especially given the one confirmed miss (`/api/group-dataclass`) suggests others may exist. Cross-browser (Firefox/WebKit) parity for this area not yet verified, chromium only so far.
...
- **Cycle 6: recommend an engineering audit of `GET /api/group-dataclass`'s missing authorization check**, and - since this was the only miss found in a 4-endpoint sample - recommend extending the same RBAC-style check across the rest of the endpoint catalog already documented in `specs/performance-sla.md`, since the fact that one endpoint was missed raises real odds that others were too. Also recommend two low-cost hardening changes with no functional impact: stop returning the raw JWTs in the login response body now that the cookies alone are confirmed sufficient, and add the `Secure` flag plus an HSTS header to the auth cookies/responses. None of these are urgent given today's actual risk profile (low-sensitivity leaked data, HTTPS-only deployment with no evident HTTP listener), but all three are inexpensive to fix and reduce defense-in-depth gaps rather than active exploits. Positive findings worth preserving as regression coverage going forward: JWT signature verification is real and correctly enforced, CORS is correctly locked to the app's own origin, and no IDOR was found on any tested cross-tenant resource.
