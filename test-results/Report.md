...
**Run type (cycle 7, 2026-07-24):** Step 8 new-scope **FULL** mini-cycle, by request ("not just performance test help explore more feuatuer you haven't reach to make test case") - a functional-coverage-gap pass distinct from performance/security. Steps 2 (`specs/planner.md` §19), 3 (live exploration of Sync/Validate-PDF/Edit dialogs, password validation, session expiry, and Logs filtering), 4 (11 new automated tests, `tests/fapa-test/{reports,markets,admins,clients,settings,auth}/073`-`081`), 5 (executed; 2 real test-authoring bugs caught and fixed - a dialog-scoping issue and a table-load race - then re-verified all passing), 6 (this report), 7 (commit) all ran.
...
- **Overall status: PASS** (93/93 passing as of cycle 2; +14 new tests in cycle 3 — 1 mail-service + 13 error-handling — all passing, bringing the suite to 107/107; +5 new performance tests added later in cycle 3, bringing the suite to 112/112; a healing pass later the same cycle brought the full suite to a clean 112/112 with zero failures on its final run; +13 new performance tests in cycle 4, bringing the suite to 125/125; +8 more performance tests same-day in cycle 4's follow-up, bringing the suite to 133/133, all passing; +19 more performance/API-endpoint-coverage tests across cycle 5's catch-up window (already executed and passing in prior sessions, see §3i); +13 new security tests in cycle 6, all passing on first run, no healing needed; +11 new functional-coverage-gap tests in cycle 7, 2 caught-and-fixed test bugs then all passing, no application defects found — bringing the suite to **177 automated tests across 92 spec files (chromium project)**, confirmed via `npx playwright test --list`).
- **Cycle 7 (2026-07-24): Functional Coverage Gaps (new scope, FULL Step 8 mini-cycle).** Surveyed every existing spec file against SCRUM.md's acceptance criteria and planner.md's own §1-16 to find scenarios flagged as "not yet automated" or missing an Edit/cancel precedent its sibling areas already had. Automated the Reports Sync and Validate-PDF dialogs' cancel paths (both genuinely destructive/irreversible actions never exercised past "Confirm" before), Currency/Team-Management/Client Edit-dialog cancel+prefill checks, Firewall's Add Range dialog, change-password mismatch/rule validation, session-token expiry (silent refresh vs. full logout), and an inverted Logs date-range filter. **No new application defects** - two minor, non-bug product observations logged (silent-disable-only feedback on invalid change-password input; no visible indication when a Logs date range is inverted) and two real test-authoring bugs were caught and fixed during this pass itself (not app issues). 11 new tests, all passing. See §3k for full detail.
...
### 3k. Cycle 7 (2026-07-24): Functional Coverage Gaps

New scope, by request: "not just performance test help explore more feuatuer you haven't reach to make test case." Surveyed every existing spec file against `user-stories/SCRUM.md`'s acceptance criteria and `specs/planner.md`'s §1-16 to find real functional gaps - not performance, not security (both already covered in cycles 4-6).

| Stage | Passed | Failed | Total |
|---|---|---|---|
| New tests (`073`-`081`, chromium, single worker) | 11 | 0 | 11 |

- **Two genuinely destructive/irreversible dialogs got their first-ever Cancel-path coverage:** Reports' "Sync" (Data synchronisation - can completely replace a client's current-month data, investigated live in 2026-07-21 but never automated for exactly that reason) and Validate PDF (a one-way "verified" state change per Business Rules, every existing report-lifecycle test only ever clicked "Confirm"). Both now exercise the full flow through to Cancel, never Confirm.
- **Three Edit-dialog cancel+prefill tests added** (Currency, Team Management, Clients) to match a pattern (ISIN already had one) that was inconsistently applied - all three confirmed live to be genuinely pre-filled forms (not blank Add-dialog copies), and none saves.
- **Firewall's Add Range dialog** (Label IP/Start IP/End IP, submit disabled until filled) had zero coverage despite Add User/Add Client/Add Currency all already having this precedent.
- **Change-password validation** (mismatched New/Re-type, and a password failing 3 of 5 live rules) confirmed the submit button correctly stays disabled in both cases - but with **no inline error text anywhere on the page**, only the disabled button itself as feedback. Not a bug (nothing in AC1 requires inline text) but worth a UX note.
- **Session/token expiry** (distinct from security/071's JWT-signature test): confirmed the app uses two cookies with different lifetimes - losing only the short-lived `token` is silently recovered via `refresh_token` with zero visible interruption to the user (a positive behavior, now regression-covered); losing both correctly redirects to `/login`.
- **Logs' inverted date range** (Start after End): confirmed the app doesn't error, doesn't show an empty-result state, and gives no visible indication the range was invalid - it silently falls back to the default unfiltered view. A minor missing-validation gap, not a crash; documented as current behavior.
- **Two real test-authoring bugs caught and fixed during this pass** (not application defects): the Sync test initially used a positional `.last()` dialog locator that silently re-targeted the wrong one of two stacked dialogs once the top one closed - fixed by scoping each dialog by its own title text instead. The Logs test initially read its "before" row count before the table had finished populating, undercounting and producing a false mismatch - fixed by waiting for a known table cell first, matching `014`'s already-established pattern.

No new application defects found in this cycle - purely coverage expansion, plus the two minor observations above. Full detail in `specs/planner.md` §19.
...
**Cycle 7 - two minor, non-bug product observations (no severity assigned - neither breaks a stated acceptance criterion):**
- **Change-password gives no inline error text when input fails validation** (mismatched New/Re-type, or missing a required character class) - the only feedback is the submit button staying disabled. A user who doesn't already know the 5 live rules by heart has no explanation for why they can't submit. Recommend a short inline hint (e.g. "Passwords don't match") alongside the existing checklist, though this is a UX polish item, not a functional defect.
- **Admins > Logs gives no feedback for an inverted date range** (Start after End) - "Apply" stays enabled and clicking it silently falls back to the default unfiltered table rather than showing an empty state or a validation message. Low impact (a user would notice the table didn't actually filter) but worth a small validation add (disable Apply, or show "Start date must be before End date") if this is ever revisited.
...
- **New area, cycle 7: Functional Coverage Gaps.** Closed several Edit-dialog/cancel-path asymmetries (Currency, Team Management, Client Edit; Firewall Add Range) and automated the cancel path of two previously-untested, genuinely destructive dialogs (Reports Sync, Validate PDF) - both had been explicitly investigated live but left unautomated in earlier cycles specifically because confirming them for real would alter shared data. Also added change-password validation, session/token-expiry, and Logs invalid-date-range coverage. 11 new tests, all passing, no new application defects. Gap: this pass covered the gaps identified in one survey pass - a similarly-thorough second pass (e.g. the ISIN/Currency Import wizards' validation, Upload's other 9 categories' edge cases beyond report-lifecycle's happy path, or a genuine Sync *Confirm* against a fully disposable client/month) would likely surface more. Chromium only so far.
...
- Cycle 7: recommend a small UX polish, not a functional fix - add inline error text to the change-password form (e.g. "Passwords don't match") so a user who doesn't already know the 5 live rules by heart isn't left guessing why the submit button won't enable. Similarly, recommend Admins > Logs either disable "Apply" or show a short message when Start date is set after End date, rather than silently falling back to the unfiltered default with no indication the range was invalid. Neither is urgent - both are low-traffic screens and neither breaks a stated acceptance criterion.
- Cycle 7: no new application defects found - this cycle was entirely coverage expansion (Sync/Validate-PDF cancel paths, 3 Edit-dialog cancel+prefill tests, Firewall Add Range, session/token-expiry). Two real test-authoring bugs were caught and fixed during the pass itself (a dialog-scoping issue in the Sync test, a table-load race in the Logs test) - both are documented in `specs/planner.md` §19 as a reminder for future dialog-stacking and table-load scenarios in this codebase, not application defects.
