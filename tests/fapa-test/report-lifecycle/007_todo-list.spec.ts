...
/**
 * Category 7 of 10: Follow-up TODO List.
 * PORTED FROM: C:\Users\khuoybo\Downloads\Project\fapa_testing (tests/007_upload_model_todolist.spec.ts).
 * Known app bug (documented in fapa_testing, reconfirmed live): the generated
 * PDF duplicates the first row's "Interlocuteur FP" value for all rows. The
 * "Interlocuteur FP" column is marked optional in uploadCategories.ts so this
 * stays visible in logs without failing the suite on it.
 *
...
const category = UPLOAD_CATEGORIES.find((c) => c.key === 'todo-list')!;
